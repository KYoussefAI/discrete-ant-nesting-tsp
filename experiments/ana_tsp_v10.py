...
def valid_case_a_route(route, number_of_cities):
    return (
...
def v9_case_a_inversion(route, rho, rng, r=None):
    number_of_cities = len(route)
    if r is None:
...
def initialize_case_a_stats(case_a_mode):
    return {
...
def finish_case_a_fallback(route, rho, rng, r, stats, mode_fallback_key):
    stats["case_a_operator_fallbacks"] = stats["case_a_operator_fallbacks"] + 1
    stats[mode_fallback_key] = stats[mode_fallback_key] + 1
    return v9_case_a_inversion(route, rho, rng, r), {
        "selected_edges": 0,
        "inserted_edges": 0,
        "failed_insertions": 0,
        "selected_memory_total": 0.0,
...
def case_a_memory_complement(route, rho, rng, memory_ranked_edges,
                             case_a_memory_escape_moves,
                             case_a_memory_top_k_edges, stats,
                             call_key, insertion_key, fallback_key,
                             invalid_key=None, fallback_r=None):
    number_of_cities = len(route)
    stats["case_a_operator_calls"] = stats["case_a_operator_calls"] + 1
    ...
    for move_number in range(case_a_memory_escape_moves):
        current_edges = route_edges(guided_route)
        candidates = []
        for ranked_edge in memory_ranked_edges:
            edge = ranked_edge[3]
            ...
            if edge in selected_this_call:
                continue
            candidates.append(ranked_edge)
            if len(candidates) >= case_a_memory_top_k_edges:
                break
        ...
        candidate_route = introduce_edge_with_reversal(guided_route, edge)
        if (
            valid_case_a_route(candidate_route, number_of_cities)
            and edge in route_edges(candidate_route)
        ):
            guided_route = candidate_route
            inserted_edges = inserted_edges + 1
    ...
    stats["case_a_operator_successes"] = stats["case_a_operator_successes"] + 1
    stats["case_a_operator_inserted_edges"] = (
        stats["case_a_operator_inserted_edges"] + inserted_edges
    )
...
def case_a_suspicious_replacement(route, rho, rng, memory_ranked_edges,
                                  edge_memory, distance_matrix,
                                  case_a_suspicious_top_k,
                                  case_a_memory_top_k_edges,
                                  case_a_suspicious_moves, stats):
    number_of_cities = len(route)
    small_epsilon = 0.000000000001
    stats["case_a_operator_calls"] = stats["case_a_operator_calls"] + 1
    stats["case_a_suspicious_calls"] = stats["case_a_suspicious_calls"] + 1
    suspicious_edges = []
    ...
    for edge in route_edges(route):
        ...
        suspicious_score = (
            ...
            / max(edge_memory[city_a][city_b], small_epsilon)
        )
        suspicious_edges.append((suspicious_score, edge))

    suspicious_edges.sort(reverse=True)
    guided_route = normalize_tour(route)
    inserted_edges = 0
    selected_memory_total = 0.0
    selected_distance_total = 0.0

    for move_number in range(case_a_suspicious_moves):
        if len(suspicious_edges[:case_a_suspicious_top_k]) == 0:
            break
        rng.choice(suspicious_edges[:case_a_suspicious_top_k])
        current_edges = route_edges(guided_route)
        candidates = []
        for ranked_edge in memory_ranked_edges:
            edge = ranked_edge[3]
    ...
    stats["case_a_suspicious_insertions"] = (
        stats["case_a_suspicious_insertions"] + inserted_edges
    )
    return guided_route, {
        "selected_edges": inserted_edges,
        "inserted_edges": inserted_edges,
...
def case_a_double_bridge(route, rho, rng, case_a_double_bridge_min_segment_size,
                         stats):
    number_of_cities = len(route)
    stats["case_a_operator_calls"] = stats["case_a_operator_calls"] + 1
    stats["case_a_double_bridge_calls"] = stats["case_a_double_bridge_calls"] + 1
    candidate_route = double_bridge_perturbation(
        route,
        case_a_double_bridge_min_segment_size,
    )

    if candidate_route is not None and valid_case_a_route(
        candidate_route,
        number_of_cities,
    ):
        stats["case_a_operator_successes"] = stats["case_a_operator_successes"] + 1
        stats["case_a_double_bridge_successes"] = (
            stats["case_a_double_bridge_successes"] + 1
        )
        return candidate_route, None
...
def build_adjacency_from_route(route):
    adjacency = {}
    number_of_cities = len(route)
    for city in route:
        adjacency[city] = set()
    for index in range(number_of_cities):
        city = route[index]
        previous_city = route[(index - 1) % number_of_cities]
        next_city = route[(index + 1) % number_of_cities]
        adjacency[city].add(previous_city)
        adjacency[city].add(next_city)
    return adjacency


def memory_built_greedy_route(number_of_cities, edge_memory, distance_matrix):
    route = [0]
    ...
    while len(unvisited) > 0:
        ...
        for city in unvisited:
            score = (
                edge_memory[current_city][city]
                / (distance_matrix[current_city][city] + 0.000000000001)
            )
...
def choose_ga_lite_parent2(best, routes, fitnesses, near_best_accepted_route,
                           edge_memory, distance_matrix, stats):
    if near_best_accepted_route is not None and near_best_accepted_route != best:
        stats["case_a_ga_lite_parent2_population"] = (
            stats["case_a_ga_lite_parent2_population"] + 1
        )
        return near_best_accepted_route.copy()

    best_other_route = None
    best_other_fitness = None
    for index in range(len(routes)):
        if routes[index] == best:
            continue
        if best_other_fitness is None or fitnesses[index] < best_other_fitness:
            best_other_route = routes[index].copy()
            best_other_fitness = fitnesses[index]

    if best_other_route is not None:
        stats["case_a_ga_lite_parent2_population"] = (
            stats["case_a_ga_lite_parent2_population"] + 1
        )
        return best_other_route

    stats["case_a_ga_lite_parent2_memory"] = (
        stats["case_a_ga_lite_parent2_memory"] + 1
    )
    return memory_built_greedy_route(len(best), edge_memory, distance_matrix)


def ga_lite_child(parent1, parent2, edge_memory, distance_matrix, rng):
    number_of_cities = len(parent1)
    adjacency1 = build_adjacency_from_route(parent1)
    adjacency2 = build_adjacency_from_route(parent2)
    combined = {}
    for city in range(number_of_cities):
        combined[city] = set()
        combined[city].update(adjacency1[city])
        combined[city].update(adjacency2[city])
    ...
    while len(unvisited) > 0:
        parent_candidates = [
            city for city in combined[current_city] if city in unvisited
        ]
        if len(parent_candidates) > 0:
            decorated = []
            for city in parent_candidates:
                degree = len([item for item in combined[city] if item in unvisited])
                memory_value = edge_memory[current_city][city]
                distance_value = distance_matrix[current_city][city]
                decorated.append(
                    (degree, -memory_value, distance_value, rng.random(), city)
                )
            decorated.sort()
            next_city = decorated[0][4]
        else:
            decorated = []
            for city in unvisited:
                score = (
                ...
                decorated.append((-score, rng.random(), city))
            decorated.sort()
            next_city = decorated[0][2]
...
def case_a_ga_lite(route, rho, rng, best, routes, fitnesses,
                   near_best_accepted_route, edge_memory, distance_matrix,
                   stats):
    number_of_cities = len(route)
    stats["case_a_operator_calls"] = stats["case_a_operator_calls"] + 1
    stats["case_a_ga_lite_calls"] = stats["case_a_ga_lite_calls"] + 1
    parent2 = choose_ga_lite_parent2(
        best,
        routes,
        fitnesses,
        near_best_accepted_route,
        edge_memory,
        distance_matrix,
        stats,
    )
    candidate_route = ga_lite_child(best, parent2, edge_memory, distance_matrix, rng)

    if valid_case_a_route(candidate_route, number_of_cities):
        stats["case_a_operator_successes"] = stats["case_a_operator_successes"] + 1
        stats["case_a_ga_lite_successes"] = stats["case_a_ga_lite_successes"] + 1
        return candidate_route, None
...
def move_ant(current, previous, best, current_fitness, previous_fitness,
             ...
             memory_top_k_edges, distance_matrix, case_a_mode,
             case_a_memory_escape_moves, case_a_memory_top_k_edges,
             case_a_safe_memory_moves, case_a_suspicious_top_k,
             case_a_suspicious_moves,
             case_a_stagnation_level_1, case_a_stagnation_level_2,
             case_a_double_bridge_min_segment_size,
             iterations_since_global_best_improvement, routes, fitnesses,
             near_best_accepted_route, case_a_stats):
    number_of_cities = len(current)
    r = random_ana_value(rng)

    if current == best:
        if case_a_mode == "v9_inversion":
            trial = v9_case_a_inversion(current, rho, rng, r)
        ...
        if case_a_mode == "memory_complement":
            trial, memory_result = case_a_memory_complement(
                current,
                rho,
                rng,
                memory_ranked_edges,
        ...
        if case_a_mode == "adaptive_memory_escape_safe":
            trial, memory_result = case_a_memory_complement(
                current,
                rho,
                rng,
        ...
        if case_a_mode == "suspicious_edge_replacement":
            trial, memory_result = case_a_suspicious_replacement(
                current,
                rho,
                rng,
                memory_ranked_edges,
        ...
        if case_a_mode == "adaptive_memory_escape":
            ...
            before_insertions = case_a_stats["case_a_operator_inserted_edges"]
            trial, memory_result = case_a_memory_complement(
                current,
                rho,
                rng,
            ...
            if (
                iterations_since_global_best_improvement >= case_a_stagnation_level_2
                and case_a_stats["case_a_operator_inserted_edges"]
                == before_insertions
            ):
                trial, memory_result = case_a_double_bridge(
                    current,
                    rho,
                    rng,
                    case_a_double_bridge_min_segment_size,
        ...
        if case_a_mode == "double_bridge_escape":
            trial, memory_result = case_a_double_bridge(
                current,
                rho,
                rng,
        ...
        if case_a_mode == "ga_edge_recombination_lite":
            trial, memory_result = case_a_ga_lite(
                current,
                rho,
                rng,
                best,
...
def run_ana(coordinates, population_size, max_iterations, seed, rho,
            ...
            case_a_mode="v9_inversion", case_a_memory_escape_moves=2,
            case_a_safe_memory_moves=1, case_a_memory_top_k_edges=20,
            case_a_suspicious_top_k=5, case_a_suspicious_moves=1,
            case_a_stagnation_level_1=500, case_a_stagnation_level_2=1500,
    ...
    case_a_stats = initialize_case_a_stats(case_a_mode)
    iterations_since_global_best_improvement = 0
    near_best_accepted_route = None
        ...
        for i in range(population_size):
            ...
            if accepted:
                ...
                if trial_fitness <= best_fitness * (1.0 + near_best_gap):
                    ...
                    memory_rank_cache["dirty"] = True
                    near_best_accepted_route = trial.copy()
                ...
                if trial_fitness < best_fitness:
                    ...
                    iterations_since_global_best_improvement = 0
                    near_best_accepted_route = best_route.copy()
                    edge_memory_deposits = (
                        edge_memory_deposits
                        + deposit_route_edges(
                            edge_memory,
